...
export default function IndustriesWeServeSection({
  ...
  industries = [
    ...
    },
    {
      ...
      icon: <HeartPulse className="w-5 h-5 sm:w-6 sm:h-6 text-white" />,
      projects: "12+",
    ...
    },
    {
      ...
      icon: <Sprout className="w-5 h-5 sm:w-6 sm:h-6 text-white" />,
      projects: "8+",
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-5 md:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5 md:gap-6 lg:gap-8">
          {industries.map((industry, index) => (
            <div
              key={index}
              className="group bg-white rounded-lg sm:rounded-xl md:rounded-2xl p-4 sm:p-5 md:p-6 border border-gray-200 hover:border-orange-300 hover:shadow-lg sm:hover:shadow-xl transition-all duration-300 hover:-translate-y-0.5 sm:hover:-translate-y-1"
            >
              {/* Header */}
              ...
              <div className="pt-3 sm:pt-4 border-t border-gray-100">
                <div className="flex flex-wrap gap-1.5 sm:gap-2">
                  {getIndustryServices(industry.name).map((service, idx) => (
                    <span
                      key={idx}
                      className="px-1.5 sm:px-2 py-1 bg-gray-50 rounded-md text-xs text-gray-700 border border-gray-200 flex items-center gap-1 min-w-0"
                    >
...
function getServiceIcon(service: string, index: number): React.ReactNode {
  const iconMap: { [key: string]: React.ReactNode } = {
    // Financial Services
    "Mobile Banking": <Smartphone className="w-3 h-3 flex-shrink-0" />,
    "Payment Systems": <CreditCard className="w-3 h-3 flex-shrink-0" />,
    "Risk Management": <Shield className="w-3 h-3 flex-shrink-0" />,
    "FinTech Apps": <Activity className="w-3 h-3 flex-shrink-0" />,

    // Healthcare
    "EHR Systems": <ClipboardCheck className="w-3 h-3 flex-shrink-0" />,
    Telemedicine: <Video className="w-3 h-3 flex-shrink-0" />,
    "Appointment Booking": <Calendar className="w-3 h-3 flex-shrink-0" />,
    "Medical Records": <Stethoscope className="w-3 h-3 flex-shrink-0" />,

    // E-commerce
    "Online Stores": <ShoppingBag className="w-3 h-3 flex-shrink-0" />,
    Marketplaces: <Store className="w-3 h-3 flex-shrink-0" />,
    "Payment Gateways": <CreditCard className="w-3 h-3 flex-shrink-0" />,
    "Inventory Management": <Package className="w-3 h-3 flex-shrink-0" />,

    // Education
    "LMS Platforms": <MonitorPlay className="w-3 h-3 flex-shrink-0" />,
    "E-learning Apps": <BookOpen className="w-3 h-3 flex-shrink-0" />,
    "Student Portals": <Users className="w-3 h-3 flex-shrink-0" />,
    "Course Management": <GraduationCap className="w-3 h-3 flex-shrink-0" />,

    // Agriculture
    "Farm Management": <Farm className="w-3 h-3 flex-shrink-0" />,
    "Supply Chain": <BarChart3 className="w-3 h-3 flex-shrink-0" />,
    Marketplace: <Store className="w-3 h-3 flex-shrink-0" />,
    "IoT Solutions": <Wifi className="w-3 h-3 flex-shrink-0" />,

    // Logistics
    "Fleet Management": <Truck className="w-3 h-3 flex-shrink-0" />,
    "Tracking Systems": <MapPin className="w-3 h-3 flex-shrink-0" />,
    "Delivery Apps": <PackageCheck className="w-3 h-3 flex-shrink-0" />,
    "Route Optimization": <Route className="w-3 h-3 flex-shrink-0" />,

    // Real Estate
    "Property Listings": <Home className="w-3 h-3 flex-shrink-0" />,
    "CRM Systems": <Users className="w-3 h-3 flex-shrink-0" />,
    "Virtual Tours": <Camera className="w-3 h-3 flex-shrink-0" />,
    "Booking Systems": <CalendarDays className="w-3 h-3 flex-shrink-0" />,

    // Manufacturing
    "ERP Systems": <Layers className="w-3 h-3 flex-shrink-0" />,
    "Inventory Control": <Package className="w-3 h-3 flex-shrink-0" />,
    "Quality Management": <ClipboardCheck className="w-3 h-3 flex-shrink-0" />,
    "Production Planning": <Gauge className="w-3 h-3 flex-shrink-0" />,

    // Default
    "Custom Solutions": <Settings className="w-3 h-3 flex-shrink-0" />,
    "Web Apps": <Globe className="w-3 h-3 flex-shrink-0" />,
    "Mobile Apps": <Smartphone className="w-3 h-3 flex-shrink-0" />,
    "Cloud Services": <Cloud className="w-3 h-3 flex-shrink-0" />,
  };

  return iconMap[service] || <div className="w-3 h-3 flex-shrink-0" />;
}